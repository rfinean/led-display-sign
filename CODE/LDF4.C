 ...
 *      ldf4.c                                                             *
 *                                                                         *
 *      Laser Display Formatter with Mail Interface                        *
 *                                                                         *
 ...
 ***************************************************************************/
	...
	const char vers[] = "v4.2,  19/4/90  R J Finean.";
...
extern char ctrlwd;             /* Printer control port byte */
...
void getmail()          /* Logs on to mail server to send spooled mail and */
			...
			/* sending next time getmail is called. Swaps the  */
			/* message on display between the two messages in  */
			/* the display ram approximately once every 30     */
			/* seconds.                                        */

			/* 20/3/90, 2/4/90  R J Finean */

{
  char txt[MAXLINELEN];         /* Miscellaneous text */
  FILE *mfp;                    /* Mailbox file pointer */

  outp(CTRLPORT, (ctrlwd ^= BANKSEL));          /* Swap message on display */
